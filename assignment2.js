...
function valInfo() {
    ...
    */
	...
	if(boolEmailCheck&&boolFirstNameCheck&&boolLastNameCheck&&boolPhoneCheck&&boolAddressCheck) {
		...
		//Costumer Info
		//
		var ContactDetails = "<h2>Customer Info:</h2>";
		ContactDetails += "<p>" +order.Name+ " "+order.LastName+ "<br/>"; 
		ContactDetails += "Email: " +order.Email+ "<br/>";
		ContactDetails += order.Address+ "<br/>";
		ContactDetails +=order.Phone+ "<br/>";
		ContactDetails += "<h2>Order</h2><br/>";
		
		
		//Printing Pizza List
		ContactDetails += "<table><tr><th> Pizza: </th><th></th><tr>";
		...
		for (i in order.DrinkList) {
			ContactDetails += "<tr><td>"+ order.DrinkList[i].quantity +" x "+order.DrinkList[i].size+", "+order.DrinkList[i].type+"<td>$"+parseFloat(order.DrinkList[i].price).toFixed(2)+"</td></tr>";
		}
		ContactDetails += "<tr><th>Order Total: </th><th>$"+parseFloat(order.Total).toFixed(2)+"</th><tr></table>";
...
function report(){
   ...
   //var sessionData = sessionStorage.getItem("Info");
	...
	for (i in orderList){
		num+=1;
		ContactDetails += "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><th style=\"width:80px\">Order: " +parseInt(num)+ "</th><th></th><th></th><th class=\"just\">"+"Completed<br/><input type=\"checkbox\" name=\"ordernum\" id=\""+i+"\" onclick=\"removeOrder()\"/></th></tr>"; 
		ContactDetails += "<tr><td style=\"width:80px\">" +orderList[i].Name+ "<br/>"; 
		...
		for (j in orderList[i].DrinkList) {
			ContactDetails += orderList[i].DrinkList[j].quantity +" x "+orderList[i].DrinkList[j].size+", "+orderList[i].DrinkList[j].type+"</td><td>$"+orderList[i].DrinkList[j].price+"</td><td></td><td></td></tr></table><hr/>";
		}
	}
...
/*function clearOrder(){
	document.getElementById("fame").value = "";	
